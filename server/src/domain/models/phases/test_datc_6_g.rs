...
fn test_datc_6_g_1() {
    ...
    let unit_e_1_nwy = a("e", "nwy");
    let unit_e_2_ska = f("e", "ska");
    let unit_r_1_swe = a("r", "swe");
    phase.units.push(unit_e_1_nwy);
    phase.units.push(unit_e_2_ska);
    phase.units.push(unit_r_1_swe);
    phase.orders.push(unit_e_1_nwy.move_to(p("swe")));
    phase.orders.push(unit_e_2_ska.convoy(unit_e_1_nwy, p("swe")));
    phase.orders.push(unit_r_1_swe.move_to(p("nwy")));
    ...
    assert!(phase.units.contains(&a("e", "swe")));
    assert!(phase.units.contains(&unit_e_2_ska));
    assert!(phase.units.contains(&a("r", "nwy")));
...
fn test_datc_6_g_2() {
    let mut phase = Phase::new_spring_main(1900, 1);
    let unit_e_nwy = a("e", "nwy");
    let unit_r_swe = f("r", "swe");
    let unit_g_ska = f("g", "ska");
    phase.units.push(unit_e_nwy);
    phase.units.push(unit_r_swe);
    phase.units.push(unit_g_ska);
    phase.orders.push(unit_e_nwy.move_to(p("swe")));
    phase.orders.push(unit_r_swe.move_to(p("nwy")));
    phase.orders.push(unit_g_ska.convoy(unit_e_nwy, p("swe")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert_eq!(phase.units.len(), 3);
    assert!(phase.units.contains(&unit_e_nwy));
    assert!(phase.units.contains(&unit_r_swe));
...
fn test_datc_6_g_4() {
    ...
    let unit_f_mid = f("f", "mid");
    let mut unit_e_eng = f("e", "eng");
    let mut unit_e_bel = a("e", "bel");
    phase.units.push(unit_f_bre);
    phase.units.push(unit_f_pic);
    phase.units.push(unit_f_bur);
    phase.units.push(unit_f_mid);
    phase.units.push(unit_e_eng);
    phase.units.push(unit_e_bel);
    phase.orders.push(unit_f_bre.move_to(p("eng")));
    phase.orders.push(unit_f_pic.move_to(p("bel")));
    phase.orders.push(unit_f_bur.support_move(unit_f_pic, p("bel")));
    phase.orders.push(unit_f_mid.support_move(unit_f_bre, p("eng")));
    phase.orders.push(unit_e_eng.convoy(unit_f_pic, p("bel")));
    phase.orders.push(unit_e_bel.move_to(p("pic")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    assert_eq!(phase.orders[3].status, OrderStatus::Valid);
    assert_eq!(phase.orders[4].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[5].status, OrderStatus::Dislodged);
    ...
    assert!(phase.units.contains(&unit_f_mid));
    assert!(phase.units.contains(&unit_e_eng.set_dislodged_from(Some(p("bre")))));
    assert!(phase.units.contains(&unit_e_bel.set_dislodged_from(Some(p("pic")))));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_5() {
    let mut phase = Phase::new_spring_main(1900, 1);
    let unit_i_rom = a("i", "rom");
    let unit_i_tyn = f("i", "tyn");
    let unit_t_apu = a("t", "apu");
    let unit_t_ion = f("t", "ion");
    phase.units.push(unit_i_rom);
    phase.units.push(unit_i_tyn);
    phase.units.push(unit_t_apu);
    phase.units.push(unit_t_ion);
    phase.orders.push(unit_i_rom.move_to(p("apu")));
    phase.orders.push(unit_i_tyn.convoy(unit_t_apu, p("rom")));
    phase.orders.push(unit_t_apu.move_to(p("rom")));
    phase.orders.push(unit_t_ion.convoy(unit_t_apu, p("rom")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    ...
    assert!(phase.units.contains(&a("i", "apu")));
    assert!(phase.units.contains(&unit_i_tyn));
    assert!(phase.units.contains(&a("t", "rom")));
    assert!(phase.units.contains(&unit_t_ion));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_6() {
    ...
    phase.units.push(unit_e_eng);
    phase.units.push(unit_g_edi);
    phase.units.push(unit_f_iri);
    phase.units.push(unit_f_nth);
    phase.units.push(unit_r_nrg);
    phase.units.push(unit_r_nat);
    phase.orders.push(unit_e_lvp.move_to(p("edi")));
    phase.orders.push(unit_e_eng.convoy(unit_e_lvp, p("edi")));
    phase.orders.push(unit_g_edi.move_to(p("lvp")));
    phase.orders.push(unit_f_iri.hold());
    phase.orders.push(unit_f_nth.hold());
    phase.orders.push(unit_r_nrg.convoy(unit_e_lvp, p("edi")));
    phase.orders.push(unit_r_nat.convoy(unit_e_lvp, p("edi")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    ...
    assert!(phase.units.contains(&a("e", "edi")));
    assert!(phase.units.contains(&unit_e_eng));
    assert!(phase.units.contains(&a("g", "lvp")));
    assert!(phase.units.contains(&unit_f_iri));
    assert!(phase.units.contains(&unit_f_nth));
    assert!(phase.units.contains(&unit_r_nrg));
    assert!(phase.units.contains(&unit_r_nat));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_7() {
    let mut phase = Phase::new_spring_main(1900, 1);
    let unit_e_ska = f("e", "ska");
    let unit_e_nwy = a("e", "nwy");
    let unit_r_swe = a("r", "swe");
    let unit_r_bot = f("r", "bot");
    phase.units.push(unit_e_ska);
    phase.units.push(unit_e_nwy);
    phase.units.push(unit_r_swe);
    phase.units.push(unit_r_bot);
    phase.orders.push(unit_e_ska.convoy(unit_r_swe, p("nwy")));
    phase.orders.push(unit_e_nwy.move_to(p("swe")));
    phase.orders.push(unit_r_swe.move_to(p("nwy")));
    phase.orders.push(unit_r_bot.convoy(unit_r_swe, p("nwy")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&unit_e_ska));
    assert!(phase.units.contains(&unit_e_nwy));
    assert!(phase.units.contains(&unit_r_swe));
    assert!(phase.units.contains(&unit_r_bot));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_8() {
    ...
    let unit_e_hol = a("e", "hol");
    phase.units.push(unit_f_bel);
    phase.units.push(unit_e_nth);
    phase.units.push(unit_e_hol);
    phase.orders.push(unit_f_bel.move_to(p("hol")).set_via_convoy());
    phase.orders.push(unit_e_nth.move_to(p("hel")));
    phase.orders.push(unit_e_hol.move_to(p("kie")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert!(phase.units.contains(&f("e", "hel")));
    assert!(phase.units.contains(&a("e", "kie")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_9() {
    ...
    let unit_e_nwy = a("e", "nwy");
    let unit_e_ska = f("e", "ska");
    let unit_e_fin = f("e", "fin");
    let unit_r_swe = a("r", "swe");
    phase.units.push(unit_e_nwy);
    phase.units.push(unit_e_ska);
    phase.units.push(unit_e_fin);
    phase.units.push(unit_r_swe);
    phase.orders.push(unit_e_nwy.move_to(p("swe")));
    phase.orders.push(unit_e_ska.convoy(unit_e_nwy, p("swe")));
    phase.orders.push(unit_e_fin.support_move(unit_e_nwy, p("swe")));
    phase.orders.push(unit_r_swe.move_to(p("nwy")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    ...
    assert!(phase.units.contains(&a("e", "swe")));
    assert!(phase.units.contains(&unit_e_ska));
    assert!(phase.units.contains(&unit_e_fin));
    assert!(phase.units.contains(&a("r", "nwy")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_10() {
    ...
    let unit_e_fin = f("e", "fin");
    let unit_g_ska = f("g", "ska");
    let mut unit_r_swe = a("r", "swe");
    let unit_r_bar = f("r", "bar");
    let unit_f_nrg = f("f", "nrg");
    let unit_f_nth = f("f", "nth");
    phase.units.push(unit_e_nwy);
    phase.units.push(unit_e_den);
    phase.units.push(unit_e_fin);
    phase.units.push(unit_g_ska);
    phase.units.push(unit_r_swe);
    phase.units.push(unit_r_bar);
    phase.units.push(unit_f_nrg);
    phase.units.push(unit_f_nth);
    phase.orders.push(unit_e_nwy.move_to(p("swe")).set_via_convoy());
    phase.orders.push(unit_e_den.support_move(unit_e_nwy, p("swe")));
    phase.orders.push(unit_e_fin.support_move(unit_e_nwy, p("swe")));
    phase.orders.push(unit_g_ska.convoy(unit_e_nwy, p("swe")));
    phase.orders.push(unit_r_swe.move_to(p("nwy")));
    phase.orders.push(unit_r_bar.support_move(unit_r_swe, p("nwy")));
    phase.orders.push(unit_f_nrg.move_to(p("nwy")));
    phase.orders.push(unit_f_nth.support_move(unit_f_nrg, p("nwy")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    ...
    assert!(phase.units.contains(&a("e", "swe")));
    assert!(phase.units.contains(&unit_e_den));
    assert!(phase.units.contains(&unit_e_fin));
    assert!(phase.units.contains(&unit_g_ska));
    assert!(phase.units.contains(&unit_r_swe.set_dislodged_via_convoy()));
    assert!(phase.units.contains(&unit_r_bar));
    assert!(phase.units.contains(&unit_f_nrg));
    assert!(phase.units.contains(&unit_f_nth));
    assert!(phase.standoff_codes.contains(&"nwy".to_string()));
}
...
fn test_datc_6_g_11() {
    let mut phase = Phase::new_spring_main(1900, 1);
    let unit_e_nwy = f("e", "nwy");
    let unit_e_nth = f("e", "nth");
    let unit_r_swe = a("r", "swe");
    let mut unit_r_ska = f("r", "ska");
    let unit_r_bar = f("r", "bar");
    phase.units.push(unit_e_nwy);
    phase.units.push(unit_e_nth);
    phase.units.push(unit_r_swe);
    phase.units.push(unit_r_ska);
    phase.units.push(unit_r_bar);
    phase.orders.push(unit_e_nwy.support_move(unit_e_nth, p("ska")));
    phase.orders.push(unit_e_nth.move_to(p("ska")));
    phase.orders.push(unit_r_swe.move_to(p("nwy")));
    phase.orders.push(unit_r_ska.convoy(unit_r_swe, p("nwy")));
    phase.orders.push(unit_r_bar.support_move(unit_r_swe, p("nwy")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    ...
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&unit_e_nwy));
    assert!(phase.units.contains(&f("e", "ska")));
    assert!(phase.units.contains(&unit_r_swe));
    assert!(phase.units.contains(&unit_r_ska.set_dislodged_from(Some(p("nth")))));
    assert!(phase.units.contains(&unit_r_bar));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_12() {
    ...
    let unit_g_eng = f("g", "eng");
    let unit_g_iri = f("g", "iri");
    phase.units.push(unit_e_lvp);
    phase.units.push(unit_e_nat);
    phase.units.push(unit_e_nrg);
    phase.units.push(unit_g_edi);
    phase.units.push(unit_g_nth);
    phase.units.push(unit_g_eng);
    phase.units.push(unit_g_iri);
    phase.orders.push(unit_e_lvp.move_to(p("edi")).set_via_convoy());
    phase.orders.push(unit_e_nat.convoy(unit_e_lvp, p("edi")));
    phase.orders.push(unit_e_nrg.convoy(unit_e_lvp, p("edi")));
    phase.orders.push(unit_g_edi.move_to(p("lvp")).set_via_convoy());
    phase.orders.push(unit_g_nth.convoy(unit_g_edi, p("lvp")));
    phase.orders.push(unit_g_eng.convoy(unit_g_edi, p("lvp")));
    phase.orders.push(unit_g_iri.convoy(unit_g_edi, p("lvp")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    ...
    assert_eq!(phase.units.len(), 7);
    assert!(phase.units.contains(&a("e", "edi")));
    assert!(phase.units.contains(&unit_e_nat));
    assert!(phase.units.contains(&unit_e_nrg));
    assert!(phase.units.contains(&a("g", "lvp")));
    assert!(phase.units.contains(&unit_g_nth));
    assert!(phase.units.contains(&unit_g_eng));
    assert!(phase.units.contains(&unit_g_iri));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_13() {
    ...
    let unit_i_alb = f("i", "alb");
    phase.units.push(unit_a_adr);
    phase.units.push(unit_a_tri);
    phase.units.push(unit_i_ven);
    phase.units.push(unit_i_alb);
    phase.orders.push(unit_a_adr.convoy(unit_a_tri, p("ven")));
    phase.orders.push(unit_a_tri.move_to(p("ven")).set_via_convoy());
    phase.orders.push(unit_i_ven.support_move(unit_i_alb, p("tri")));
    phase.orders.push(unit_i_alb.move_to(p("tri")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    ...
    assert_eq!(phase.orders[3].status, OrderStatus::Success);
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&unit_a_adr));
    assert!(phase.units.contains(&unit_a_tri.set_dislodged_from(Some(p("alb")))));
    assert!(phase.units.contains(&unit_i_ven));
    assert!(phase.units.contains(&f("i", "tri")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_14() {
    ...
    phase.orders.push(unit_f_nrg.move_to(p("nwy")));
    phase.orders.push(unit_f_nth.support_move(unit_f_nrg, p("nwy")));
    phase.orders.push(unit_g_ska.convoy(unit_r_swe, p("nwy")));
    phase.orders.push(unit_r_swe.move_to(p("nwy")).set_via_convoy());
    phase.orders.push(unit_r_bar.support_move(unit_r_swe, p("nwy")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    ...
    assert!(phase.units.contains(&unit_f_nth));
    assert!(phase.units.contains(&unit_g_ska));
    assert!(phase.units.contains(&unit_r_swe.set_dislodged_from(Some(p("nwy")))));
    assert!(phase.units.contains(&unit_r_bar));
    assert!(phase.standoff_codes.contains(&"nwy".to_string()));
}
...
fn test_datc_6_g_15() {
    ...
    phase.orders.push(unit_e_hol.support_move(unit_e_lon, p("bel")));
    phase.orders.push(unit_e_yor.move_to(p("lon")));
    phase.orders.push(unit_e_lon.move_to(p("bel")).set_via_convoy());
    phase.orders.push(unit_f_eng.convoy(unit_f_bel, p("lon")));
    phase.orders.push(unit_f_bel.move_to(p("lon")).set_via_convoy());
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Failure);
    ...
    assert!(phase.units.contains(&unit_e_hol));
    assert!(phase.units.contains(&unit_e_yor));
    assert!(phase.units.contains(&a("e", "bel")));
    assert!(phase.units.contains(&unit_f_eng));
    assert!(phase.units.contains(&unit_f_bel.set_dislodged_via_convoy()));
    assert!(phase.standoff_codes.contains(&"lon".to_string()));
}
...
fn test_datc_6_g_16() {
    ...
    phase.orders.push(unit_e_nwy.move_to(p("swe")));
    phase.orders.push(unit_e_den.support_move(unit_e_nwy, p("swe")));
    phase.orders.push(unit_e_bal.support_move(unit_e_nwy, p("swe")));
    phase.orders.push(unit_e_nth.move_to(p("nwy")));
    phase.orders.push(unit_r_swe.move_to(p("nwy")).set_via_convoy());
    phase.orders.push(unit_r_ska.convoy(unit_r_swe, p("nwy")));
    phase.orders.push(unit_r_nrg.support_move(unit_r_swe, p("nwy")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    ...
    assert!(phase.units.contains(&a("e", "swe")));
    assert!(phase.units.contains(&unit_e_den));
    assert!(phase.units.contains(&unit_e_bal));
    assert!(phase.units.contains(&unit_e_nth));
    assert!(phase.units.contains(&a("r", "nwy")));
    assert!(phase.units.contains(&unit_r_ska));
    assert!(phase.units.contains(&unit_r_nrg));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_18() {
    ...
    let unit_f_eng = f("f", "eng");
    let unit_f_bel = a("f", "bel");
    let unit_f_wal = a("f", "wal");
    phase.units.push(unit_e_nth);
    phase.units.push(unit_e_hol);
    phase.units.push(unit_e_yor);
    phase.units.push(unit_e_lon);
    phase.units.push(unit_e_ruh);
    phase.units.push(unit_f_eng);
    phase.units.push(unit_f_bel);
    phase.units.push(unit_f_wal);
    phase.orders.push(unit_e_nth.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_e_hol.support_move(unit_e_lon, p("bel")));
    phase.orders.push(unit_e_yor.move_to(p("lon")));
    phase.orders.push(unit_e_lon.move_to(p("bel")));
    phase.orders.push(unit_e_ruh.support_move(unit_e_lon, p("bel")));
    phase.orders.push(unit_f_eng.convoy(unit_f_bel, p("lon")));
    phase.orders.push(unit_f_bel.move_to(p("lon")));
    phase.orders.push(unit_f_wal.support_move(unit_f_bel, p("lon")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Failure);
    ...
    assert!(phase.units.contains(&unit_e_yor));
    assert!(phase.units.contains(&a("e", "bel")));
    assert!(phase.units.contains(&unit_e_ruh));
    assert!(phase.units.contains(&unit_f_eng));
    assert!(phase.units.contains(&a("f", "lon")));
    assert!(phase.units.contains(&unit_f_wal));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_19() {
    ...
    let unit_i_spa = a("i", "spa");
    phase.units.push(unit_f_mar);
    phase.units.push(unit_f_wes);
    phase.units.push(unit_i_gol);
    phase.units.push(unit_i_spa);
    phase.orders.push(unit_f_mar.move_to(p("spa")));
    phase.orders.push(unit_f_wes.convoy(unit_f_mar, p("spa")));
    phase.orders.push(unit_i_gol.convoy(unit_f_mar, p("spa")));
    phase.orders.push(unit_i_spa.move_to(p("mar")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    ...
    assert!(phase.units.contains(&unit_f_wes));
    assert!(phase.units.contains(&unit_i_gol));
    assert!(phase.units.contains(&unit_i_spa));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_g_20() {
    ...
    let unit_f_pic = a("f", "pic");
    let unit_f_bur = a("f", "bur");
    let unit_a_mid = f("f", "mid");
    let mut unit_a_eng = f("e", "eng");
    phase.units.push(unit_f_bre);
    phase.units.push(unit_f_pic);
    phase.units.push(unit_f_bur);
    phase.units.push(unit_a_mid);
    phase.units.push(unit_a_eng);
    phase.orders.push(unit_f_bre.move_to(p("eng")));
    phase.orders.push(unit_f_pic.move_to(p("bel")).set_via_convoy());
    phase.orders.push(unit_f_bur.support_move(unit_f_pic, p("bel")));
    phase.orders.push(unit_a_mid.support_move(unit_f_bre, p("eng")));
    phase.orders.push(unit_a_eng.convoy(unit_f_pic, p("bel")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    ...
    assert!(phase.units.contains(&unit_f_bur));
    assert!(phase.units.contains(&unit_a_mid));
    assert!(phase.units.contains(&unit_a_eng.set_dislodged_from(Some(p("bre")))));
    assert!(phase.standoff_codes.is_empty());
}
