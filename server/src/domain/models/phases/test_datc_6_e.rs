...
fn test_datc_6_e_1() {
    ...
    assert!(phase.units.contains(&f("g", "ber")));
    assert!(phase.units.contains(&unit_g_sil));
    assert!(phase.units.contains(&unit_r_pru.set_dislodged_from(Some(p("ber")))));
...
fn test_datc_6_e_2() {
    ...
    phase.orders.push(unit_g_ber.move_to(p("kie")));
    phase.orders.push(unit_g_kie.move_to(p("ber")));
    phase.orders.push(unit_g_mun.support_move(unit_g_ber, p("kie")));
    resolve_orders_for_main_phase(&mut phase);
...
fn test_datc_6_e_3() {
    ...
    phase.units.push(unit_g_ber);
    phase.units.push(unit_g_mun);
    phase.units.push(unit_e_kie);
    phase.orders.push(unit_g_ber.move_to(p("kie")));
    phase.orders.push(unit_g_mun.support_move(unit_e_kie, p("ber")));
    phase.orders.push(unit_e_kie.move_to(p("ber")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    ...
    assert!(phase.units.contains(&unit_g_ber));
    assert!(phase.units.contains(&unit_g_mun));
    assert!(phase.units.contains(&unit_e_kie));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_e_4() {
    ...
    phase.units.push(unit_a_kie);
    phase.units.push(unit_a_ruh);
    phase.orders.push(unit_g_hol.move_to(p("nth")));
    phase.orders.push(unit_g_hel.support_move(unit_g_hol, p("nth")));
    phase.orders.push(unit_g_ska.support_move(unit_g_hol, p("nth")));
    phase.orders.push(unit_f_nth.move_to(p("hol")));
    phase.orders.push(unit_f_bel.support_move(unit_f_nth, p("hol")));
    phase.orders.push(unit_e_edi.support_move(unit_e_nrg, p("nth")));
    phase.orders.push(unit_e_yor.support_move(unit_e_nrg, p("nth")));
    phase.orders.push(unit_e_nrg.move_to(p("nth")));
    phase.orders.push(unit_a_kie.support_move(unit_a_ruh, p("hol")));
    phase.orders.push(unit_a_ruh.move_to(p("hol")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    ...
    assert_eq!(phase.orders[7].status, OrderStatus::Failure);
    assert_eq!(phase.orders[8].status, OrderStatus::Valid);
    ...
    assert!(phase.units.contains(&unit_a_kie));
    assert!(phase.units.contains(&unit_a_ruh));
...
fn test_datc_6_e_5() {
    ...
    phase.orders.push(unit_e_yor.support_move(unit_e_nrg, p("nth")));
    phase.orders.push(unit_e_nrg.move_to(p("nth")));
    phase.orders.push(unit_e_lon.support_move(unit_e_nrg, p("nth")));
    phase.orders.push(unit_a_kie.support_move(unit_a_ruh, p("hol")));
    phase.orders.push(unit_a_ruh.move_to(p("hol")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    ...
    assert_eq!(phase.orders[5].status, OrderStatus::Valid);
    assert_eq!(phase.orders[6].status, OrderStatus::Valid);
    assert_eq!(phase.orders[7].status, OrderStatus::Success);
    assert_eq!(phase.orders[8].status, OrderStatus::Valid);
    assert_eq!(phase.orders[9].status, OrderStatus::Valid);
    assert_eq!(phase.orders[10].status, OrderStatus::Failure);
    assert_eq!(phase.units.len(), 11);
    assert!(phase.units.contains(&unit_g_hol));
    assert!(phase.units.contains(&unit_g_hel));
    assert!(phase.units.contains(&unit_g_ska));
    assert!(phase.units.contains(&unit_f_nth.set_dislodged_from(Some(p("nrg")))));
    assert!(phase.units.contains(&unit_f_bel));
    assert!(phase.units.contains(&unit_e_edi));
    assert!(phase.units.contains(&unit_e_yor));
...
fn test_datc_6_e_6() {
    ...
    phase.orders.push(unit_f_nth.move_to(p("hol")));
    phase.orders.push(unit_f_bel.support_move(unit_f_nth, p("hol")));
    phase.orders.push(unit_f_eng.support_move(unit_g_hol, p("nth")));
    phase.orders.push(unit_a_kie.support_move(unit_a_ruh, p("hol")));
    phase.orders.push(unit_a_ruh.move_to(p("hol")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
...
fn test_datc_6_e_7() {
    ...
    phase.orders.push(unit_e_nth.hold());
    phase.orders.push(unit_e_yor.support_move(unit_r_nwy, p("nth")));
    phase.orders.push(unit_g_hol.support_move(unit_g_hel, p("nth")));
    phase.orders.push(unit_g_hel.move_to(p("nth")));
    phase.orders.push(unit_r_ska.support_move(unit_r_nwy, p("nth")));
    phase.orders.push(unit_r_nwy.move_to(p("nth")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
...
fn test_datc_6_e_9() {
    ...
    phase.units.push(unit_r_ska);
    phase.units.push(unit_r_nwy);
    phase.orders.push(unit_e_nth.move_to(p("nrg")));
    phase.orders.push(unit_e_yor.support_move(unit_r_nwy, p("nth")));
    phase.orders.push(unit_g_hol.support_move(unit_g_hel, p("nth")));
    phase.orders.push(unit_g_hel.move_to(p("nth")));
    phase.orders.push(unit_r_ska.support_move(unit_r_nwy, p("nth")));
    ...
    assert_eq!(phase.units.len(), 6);
    assert!(phase.units.contains(&f("e", "nrg")));
    assert!(phase.units.contains(&unit_e_yor));
    assert!(phase.units.contains(&unit_g_hol));
    assert!(phase.units.contains(&unit_g_hel));
    assert!(phase.units.contains(&unit_r_ska));
    assert!(phase.units.contains(&f("r", "nth")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_e_10() {
    ...
    phase.units.push(unit_r_ska);
    phase.units.push(unit_r_nwy);
    phase.orders.push(unit_e_nth.move_to(p("den")));
    phase.orders.push(unit_e_yor.support_move(unit_r_nwy, p("nth")));
    phase.orders.push(unit_g_hol.support_move(unit_g_hel, p("nth")));
    phase.orders.push(unit_g_hel.move_to(p("nth")));
    phase.orders.push(unit_g_den.move_to(p("hel")));
    phase.orders.push(unit_r_ska.support_move(unit_r_nwy, p("nth")));
    phase.orders.push(unit_r_nwy.move_to(p("nth")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
...
fn test_datc_6_e_11() {
    ...
    phase.units.push(unit_f_spa);
    phase.units.push(unit_f_mid);
    phase.units.push(unit_f_gol);
    phase.units.push(unit_g_mar);
    phase.units.push(unit_g_gas);
    phase.units.push(unit_i_por);
    phase.units.push(unit_i_wes);
    phase.orders.push(unit_f_spa.move_to(p("por")).set_via_convoy());
    phase.orders.push(unit_f_mid.convoy(unit_f_spa, p("por")));
    phase.orders.push(unit_f_gol.support_move(unit_i_por, p("spa_nc")));
    phase.orders.push(unit_g_mar.support_move(unit_g_gas, p("spa")));
    phase.orders.push(unit_g_gas.move_to(p("spa")));
    phase.orders.push(unit_i_por.move_to(p("spa_nc")));
    phase.orders.push(unit_i_wes.support_move(unit_i_por, p("spa_nc")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    ...
    assert_eq!(phase.orders[6].status, OrderStatus::Valid);
    assert_eq!(phase.units.len(), 7);
    assert!(phase.units.contains(&a("f", "por")));
    assert!(phase.units.contains(&unit_f_mid));
    assert!(phase.units.contains(&unit_f_gol));
    assert!(phase.units.contains(&unit_g_mar));
    assert!(phase.units.contains(&unit_g_gas));
    assert!(phase.units.contains(&f("i", "spa_nc")));
...
fn test_datc_6_e_12() {
    ...
    phase.units.push(unit_a_bud);
    phase.units.push(unit_a_ser);
    phase.units.push(unit_i_vie);
    phase.units.push(unit_r_gal);
    phase.units.push(unit_r_rum);
    phase.orders.push(unit_a_bud.move_to(p("rum")));
    phase.orders.push(unit_a_ser.support_move(unit_i_vie, p("bud")));
    phase.orders.push(unit_i_vie.move_to(p("bud")));
    phase.orders.push(unit_r_gal.move_to(p("bud")));
    phase.orders.push(unit_r_rum.support_move(unit_r_gal, p("bud")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Failure);
    ...
    assert!(phase.units.contains(&unit_a_bud));
    assert!(phase.units.contains(&unit_a_ser));
    assert!(phase.units.contains(&unit_i_vie));
    assert!(phase.units.contains(&unit_r_gal));
    assert!(phase.units.contains(&unit_r_rum));
    assert!(phase.standoff_codes.contains(&"bud".to_string()));
}
...
fn test_datc_6_e_13() {
    ...
    phase.units.push(unit_r_nwy);
    phase.orders.push(unit_e_edi.support_move(unit_e_yor, p("nth")));
    phase.orders.push(unit_e_yor.move_to(p("nth")));
    phase.orders.push(unit_f_bel.move_to(p("nth")));
    phase.orders.push(unit_f_eng.support_move(unit_f_bel, p("nth")));
    phase.orders.push(unit_g_nth.hold());
    phase.orders.push(unit_r_nrg.move_to(p("nth")));
    phase.orders.push(unit_r_nwy.support_move(unit_r_nrg, p("nth")));
    resolve_orders_for_main_phase(&mut phase);
...
fn test_datc_6_e_14() {
    ...
    phase.orders.push(unit_r_edi.move_to(p("lvp")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
...
fn test_datc_6_e_15() {
    ...
    phase.units.push(unit_f_kie);
    phase.units.push(unit_f_mun);
    phase.units.push(unit_f_sil);
    phase.units.push(unit_g_ber);
    phase.units.push(unit_g_den);
    phase.units.push(unit_g_hel);
    phase.units.push(unit_r_bal);
    phase.units.push(unit_r_pru);
    phase.orders.push(unit_e_hol.support_move(unit_e_ruh, p("kie")));
    phase.orders.push(unit_e_ruh.move_to(p("kie")));
    phase.orders.push(unit_f_kie.move_to(p("ber")));
    phase.orders.push(unit_f_mun.support_move(unit_f_kie, p("ber")));
    phase.orders.push(unit_f_sil.support_move(unit_f_kie, p("ber")));
    phase.orders.push(unit_g_ber.move_to(p("kie")));
    phase.orders.push(unit_g_den.support_move(unit_g_ber, p("kie")));
    phase.orders.push(unit_g_hel.support_move(unit_g_ber, p("kie")));
    ...
    assert!(phase.units.contains(&unit_f_kie));
    assert!(phase.units.contains(&unit_f_mun));
    assert!(phase.units.contains(&unit_f_sil));
    assert!(phase.units.contains(&unit_g_ber));
    assert!(phase.units.contains(&unit_g_den));
    assert!(phase.units.contains(&unit_g_hel));
