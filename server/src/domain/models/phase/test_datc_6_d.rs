...
fn test_datc_6_d_1() {
    ...
    phase.orders.push(unit_a_adr.support_move(unit_a_tri, p("ven")));
    phase.orders.push(unit_a_tri.move_to(p("ven")));
    phase.orders.push(unit_i_ven.hold());
    phase.orders.push(unit_i_tyr.support_hold(unit_i_ven));
    ...
    assert!(phase.units.contains(&unit_a_tri));
...
fn test_datc_6_d_2() {
    ...
    assert_eq!(phase.orders[4].status, OrderStatus::Cut);
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&unit_a_adr));
    assert!(phase.units.contains(&a("a", "ven")));
    assert!(phase.units.contains(&unit_a_vie));
    assert!(phase.units.contains(&unit_i_ven.set_dislodged_from(Some(p("tri")))));
    assert!(phase.units.contains(&unit_i_tyr));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_3() {
    ...
    phase.orders.push(unit_a_tri.move_to(p("ven")));
    phase.orders.push(unit_i_ven.hold());
    phase.orders.push(unit_i_ion.move_to(p("adr")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Cut);
    assert_eq!(phase.orders[1].status, OrderStatus::Failure);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    ...
    assert!(phase.units.contains(&unit_a_adr));
    assert!(phase.units.contains(&unit_a_tri));
    assert!(phase.units.contains(&a("i", "ven")));
    assert!(phase.units.contains(&unit_i_ion));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_4() {
    ...
    phase.units.push(unit_g_ber);
    phase.units.push(unit_g_kie);
    phase.units.push(unit_r_bal);
    phase.units.push(unit_r_pru);
    phase.orders.push(unit_g_ber.support_hold(unit_g_kie));
    phase.orders.push(unit_g_kie.support_hold(unit_g_ber));
    phase.orders.push(unit_r_bal.support_move(unit_r_pru, p("ber")));
    phase.orders.push(unit_r_pru.move_to(p("ber")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Cut);
    ...
    assert_eq!(phase.orders[3].status, OrderStatus::Failure);
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&unit_g_ber));
    assert!(phase.units.contains(&unit_g_kie));
    assert!(phase.units.contains(&unit_r_bal));
    assert!(phase.units.contains(&unit_r_pru));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_5() {
    ...
    phase.units.push(unit_r_bal);
    phase.units.push(unit_r_pru);
    phase.orders.push(unit_g_ber.support_move(unit_g_mun, p("sil")));
    phase.orders.push(unit_g_kie.support_hold(unit_g_ber));
    phase.orders.push(unit_g_mun.move_to(p("sil")));
    phase.orders.push(unit_r_bal.support_move(unit_r_pru, p("ber")));
    phase.orders.push(unit_r_pru.move_to(p("ber")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Cut);
    ...
    assert!(phase.units.contains(&unit_g_ber));
    assert!(phase.units.contains(&unit_g_kie));
    assert!(phase.units.contains(&a("g", "sil")));
    assert!(phase.units.contains(&unit_r_bal));
    assert!(phase.units.contains(&unit_r_pru));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_6() {
    ...
    phase.units.push(unit_g_ber);
    phase.units.push(unit_g_bal);
    phase.units.push(unit_g_pru);
    phase.units.push(unit_r_lvn);
    phase.units.push(unit_r_bot);
    phase.orders.push(unit_g_ber.move_to(p("swe")));
    phase.orders.push(unit_g_bal.convoy(unit_g_ber, p("swe")));
    phase.orders.push(unit_g_pru.support_hold(unit_g_bal));
    phase.orders.push(unit_r_lvn.move_to(p("bal")));
    phase.orders.push(unit_r_bot.support_move(unit_r_lvn, p("bal")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&a("g", "swe")));
    assert!(phase.units.contains(&unit_g_bal));
    assert!(phase.units.contains(&unit_g_pru));
    assert!(phase.units.contains(&unit_r_lvn));
...
fn test_datc_6_d_7() {
    ...
    phase.units.push(unit_r_lvn);
    phase.units.push(unit_r_bot);
    phase.units.push(unit_r_fin);
    phase.orders.push(unit_g_bal.move_to(p("swe")));
    phase.orders.push(unit_g_pru.support_hold(unit_g_bal));
    phase.orders.push(unit_r_lvn.move_to(p("bal")));
    phase.orders.push(unit_r_bot.support_move(unit_r_lvn, p("bal")));
    phase.orders.push(unit_r_fin.move_to(p("swe")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert_eq!(phase.orders[4].status, OrderStatus::Failure);
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&unit_g_bal.set_dislodged_from(Some(p("lvn")))));
    assert!(phase.units.contains(&unit_g_pru));
    assert!(phase.units.contains(&f("r", "bal")));
    assert!(phase.units.contains(&unit_r_bot));
    assert!(phase.units.contains(&unit_r_fin));
    assert!(phase.standoff_codes.contains(&"swe".to_string()));
}
...
fn test_datc_6_d_8() {
    ...
    phase.units.push(unit_a_ion);
    phase.units.push(unit_a_ser);
    phase.units.push(unit_a_alb);
    phase.units.push(unit_t_gre);
    phase.units.push(unit_t_bul);
    phase.orders.push(unit_a_ion.hold());
    phase.orders.push(unit_a_ser.support_move(unit_a_alb, p("gre")));
    phase.orders.push(unit_a_alb.move_to(p("gre")));
    phase.orders.push(unit_t_gre.move_to(p("nap")));
    phase.orders.push(unit_t_bul.support_hold(unit_t_gre));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    assert_eq!(phase.orders[3].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[4].status, OrderStatus::Invalid);
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&unit_a_ion));
    assert!(phase.units.contains(&unit_a_ser));
    assert!(phase.units.contains(&a("a", "gre")));
    assert!(phase.units.contains(&unit_t_gre.set_dislodged_from(Some(p("alb")))));
    assert!(phase.units.contains(&unit_t_bul));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_9() {
    ...
    phase.orders.push(unit_a_alb.support_move(unit_a_tri, p("ser")));
    phase.orders.push(unit_a_tri.hold());
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    ...
    assert!(phase.units.contains(&unit_i_tyr));
    assert!(phase.units.contains(&unit_a_alb));
...
fn test_datc_6_d_10() {
    ...
    phase.units.push(unit_g_kie);
    phase.units.push(unit_g_mun);
    phase.orders.push(unit_g_ber.hold());
    phase.orders.push(unit_g_kie.move_to(p("ber")));
    phase.orders.push(unit_g_mun.support_move(unit_g_kie, p("ber")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Failure);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
...
fn test_datc_6_d_11() {
    ...
    phase.units.push(unit_g_kie);
    phase.units.push(unit_g_mun);
    phase.units.push(unit_r_war);
    phase.orders.push(unit_g_ber.move_to(p("pru")));
    phase.orders.push(unit_g_kie.move_to(p("ber")));
    phase.orders.push(unit_g_mun.support_move(unit_g_kie, p("ber")));
    phase.orders.push(unit_r_war.move_to(p("pru")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert!(phase.units.contains(&unit_g_kie));
    assert!(phase.units.contains(&unit_g_mun));
    assert!(phase.units.contains(&unit_r_war));
    assert!(phase.standoff_codes.contains(&"pru".to_string()));
}
...
fn test_datc_6_d_12() {
    ...
    phase.units.push(unit_i_ven);
    phase.orders.push(unit_a_tri.hold());
    phase.orders.push(unit_a_vie.support_move(unit_i_ven, p("tri")));
    phase.orders.push(unit_i_ven.move_to(p("tri")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
...
fn test_datc_6_d_13() {
    ...
    phase.orders.push(unit_a_tri.move_to(p("adr")));
    phase.orders.push(unit_a_vie.support_move(unit_i_ven, p("tri")));
    phase.orders.push(unit_i_ven.move_to(p("tri")));
    phase.orders.push(unit_i_apu.move_to(p("adr")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Failure);
    ...
    assert!(phase.standoff_codes.contains(&"adr".to_string()));
}
...
fn test_datc_6_d_14() {
    ...
    phase.units.push(unit_i_ven);
    phase.units.push(unit_i_tyr);
    phase.units.push(unit_i_adr);
    phase.orders.push(unit_a_tri.hold());
    phase.orders.push(unit_a_vie.support_move(unit_i_ven, p("tri")));
    phase.orders.push(unit_i_ven.move_to(p("tri")));
    phase.orders.push(unit_i_tyr.support_move(unit_i_ven, p("tri")));
    phase.orders.push(unit_i_adr.support_move(unit_i_ven, p("tri")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    ...
    assert!(phase.units.contains(&a("i", "tri")));
    assert!(phase.units.contains(&unit_i_tyr));
    assert!(phase.units.contains(&unit_i_adr));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_15() {
    ...
    phase.units.push(unit_r_con);
    phase.units.push(unit_r_bla);
    phase.units.push(unit_t_ank);
    phase.orders.push(unit_r_con.support_move(unit_r_bla, p("ank")));
    phase.orders.push(unit_r_bla.move_to(p("ank")));
    phase.orders.push(unit_t_ank.move_to(p("con")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Dislodged);
    assert_eq!(phase.units.len(), 3);
    assert!(phase.units.contains(&unit_r_con));
    assert!(phase.units.contains(&f("r", "ank")));
    assert!(phase.units.contains(&unit_t_ank.set_dislodged_from(Some(p("bla")))));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_16() {
    ...
    phase.orders.push(unit_e_lon.hold());
    phase.orders.push(unit_e_nth.convoy(unit_f_bel, p("lon")));
    phase.orders.push(unit_f_eng.support_move(unit_f_bel, p("lon")));
    ...
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&unit_e_lon.set_dislodged_from(Some(p("bel")))));
...
fn test_datc_6_d_17() {
    ...
    phase.units.push(unit_r_bla);
    phase.units.push(unit_t_ank);
    phase.units.push(unit_t_smy);
    phase.units.push(unit_t_arm);
    phase.orders.push(unit_r_con.support_move(unit_r_bla, p("ank")));
    phase.orders.push(unit_r_bla.move_to(p("ank")));
    phase.orders.push(unit_t_ank.move_to(p("con")));
    phase.orders.push(unit_t_smy.support_move(unit_t_ank, p("con")));
    phase.orders.push(unit_t_arm.move_to(p("ank")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[1].status, OrderStatus::Failure);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    ...
    assert_eq!(phase.orders[4].status, OrderStatus::Failure);
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&unit_r_con.set_dislodged_from(Some(p("ank")))));
    assert!(phase.units.contains(&unit_r_bla));
    assert!(phase.units.contains(&f("t", "con")));
    assert!(phase.units.contains(&unit_t_smy));
    assert!(phase.units.contains(&unit_t_arm));
    assert!(phase.standoff_codes.contains(&"ank".to_string()));
}
...
fn test_datc_6_d_18() {
    ...
    phase.units.push(unit_r_con);
    phase.units.push(unit_r_bla);
    phase.units.push(unit_r_bul);
    phase.units.push(unit_t_ank);
    phase.units.push(unit_t_smy);
    phase.units.push(unit_t_arm);
    phase.orders.push(unit_r_con.support_move(unit_r_bla, p("ank")));
    phase.orders.push(unit_r_bla.move_to(p("ank")));
    phase.orders.push(unit_r_bul.support_hold(unit_r_con));
    phase.orders.push(unit_t_ank.move_to(p("con")));
    phase.orders.push(unit_t_smy.support_move(unit_t_ank, p("con")));
    phase.orders.push(unit_t_arm.move_to(p("ank")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert!(phase.units.contains(&unit_r_con));
    assert!(phase.units.contains(&f("r", "ank")));
    assert!(phase.units.contains(&unit_r_bul));
    assert!(phase.units.contains(&unit_t_ank.set_dislodged_from(Some(p("bla")))));
    assert!(phase.units.contains(&unit_t_smy));
    assert!(phase.units.contains(&unit_t_arm));
    assert!(phase.standoff_codes.is_empty());
...
fn test_datc_6_d_19() {
    ...
    phase.units.push(unit_r_con);
    phase.units.push(unit_r_bla);
    phase.units.push(unit_r_smy);
    phase.units.push(unit_t_ank);
    phase.orders.push(unit_r_con.support_move(unit_r_bla, p("ank")));
    phase.orders.push(unit_r_bla.move_to(p("ank")));
    phase.orders.push(unit_r_smy.support_move(unit_t_ank, p("con")));
    phase.orders.push(unit_t_ank.move_to(p("con")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    ...
    assert!(phase.units.contains(&unit_r_con));
    assert!(phase.units.contains(&f("r", "ank")));
    assert!(phase.units.contains(&unit_r_smy));
    assert!(phase.units.contains(&unit_t_ank.set_dislodged_from(Some(p("bla")))));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_20() {
    ...
    phase.units.push(unit_f_eng);
    phase.orders.push(unit_e_lon.support_move(unit_e_nth, p("eng")));
    phase.orders.push(unit_e_nth.move_to(p("eng")));
    phase.orders.push(unit_e_yor.move_to(p("lon")));
    phase.orders.push(unit_f_eng.hold());
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Failure);
    ...
    assert!(phase.units.contains(&unit_f_eng.set_dislodged_from(Some(p("nth")))));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_21() {
    ...
    phase.units.push(unit_i_tyr);
    phase.units.push(unit_g_mun);
    phase.units.push(unit_r_sil);
    phase.units.push(unit_r_ber);
    phase.orders.push(unit_a_tri.hold());
    phase.orders.push(unit_i_ven.move_to(p("tri")));
    phase.orders.push(unit_i_tyr.support_move(unit_i_ven, p("tri")));
    phase.orders.push(unit_g_mun.move_to(p("tyr")));
    phase.orders.push(unit_r_sil.move_to(p("mun")));
    phase.orders.push(unit_r_ber.support_move(unit_r_sil, p("mun")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Failure);
    ...
    assert!(phase.units.contains(&unit_i_ven));
    assert!(phase.units.contains(&unit_i_tyr));
    assert!(phase.units.contains(&unit_g_mun.set_dislodged_from(Some(p("sil")))));
    assert!(phase.units.contains(&a("r", "mun")));
    assert!(phase.units.contains(&unit_r_ber));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_22() {
    ...
    let unit_r_ber = a("r", "ber");
    phase.units.push(unit_g_kie);
    phase.units.push(unit_g_bur);
    phase.units.push(unit_r_mun);
    phase.units.push(unit_r_ber);
    phase.orders.push(unit_g_kie.move_to(p("mun")));
    phase.orders.push(unit_g_bur.support_move(unit_g_kie, p("mun")));
    phase.orders.push(unit_r_mun.move_to(p("kie")));
    phase.orders.push(unit_r_ber.support_move(unit_r_mun, p("kie")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[1].status, OrderStatus::Invalid);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    assert_eq!(phase.orders[3].status, OrderStatus::Valid);
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&unit_g_kie.set_dislodged_from(Some(p("mun")))));
    assert!(phase.units.contains(&unit_g_bur));
    assert!(phase.units.contains(&a("r", "kie")));
    assert!(phase.units.contains(&unit_r_ber));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_23() {
    ...
    phase.orders.push(unit_i_wes.support_move(unit_i_gol, p("spa_sc")));
    phase.orders.push(unit_f_spa_nc.move_to(p("gol")));
    phase.orders.push(unit_f_mar.support_move(unit_f_spa_nc, p("gol")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Dislodged);
    ...
    assert!(phase.units.contains(&unit_i_wes));
    assert!(phase.units.contains(&unit_f_spa_nc.set_dislodged_from(Some(p("gol")))));
...
fn test_datc_6_d_24() {
    ...
    phase.units.push(unit_f_mar);
    phase.units.push(unit_f_spa_sc);
    phase.units.push(unit_i_gol);
    phase.units.push(unit_t_tyr);
    phase.units.push(unit_t_wes);
    phase.orders.push(unit_f_mar.move_to(p("gol")));
    phase.orders.push(unit_f_spa_sc.support_move(unit_f_mar, p("gol")));
    phase.orders.push(unit_i_gol.hold());
    phase.orders.push(unit_t_tyr.support_move(unit_t_wes, p("gol")));
    phase.orders.push(unit_t_wes.move_to(p("gol")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&unit_f_mar));
    assert!(phase.units.contains(&unit_f_spa_sc));
    assert!(phase.units.contains(&unit_i_gol.set_dislodged_from(Some(p("wes")))));
    assert!(phase.units.contains(&unit_t_tyr));
    assert!(phase.units.contains(&f("t", "gol")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_25() {
    ...
    phase.units.push(unit_r_bal);
    phase.units.push(unit_r_pru);
    phase.orders.push(unit_g_ber.support_hold(unit_r_pru));
    phase.orders.push(unit_g_kie.support_hold(unit_g_ber));
    phase.orders.push(unit_r_bal.support_move(unit_r_pru, p("ber")));
    phase.orders.push(unit_r_pru.move_to(p("ber")));
    resolve_orders_for_main_phase(&mut phase);
...
fn test_datc_6_d_26() {
    ...
    phase.units.push(unit_r_bal);
    phase.units.push(unit_r_pru);
    phase.orders.push(unit_g_ber.support_move(unit_r_pru, p("sil")));
    phase.orders.push(unit_g_kie.support_hold(unit_g_ber));
    phase.orders.push(unit_r_bal.support_move(unit_r_pru, p("ber")));
    phase.orders.push(unit_r_pru.move_to(p("ber")));
    resolve_orders_for_main_phase(&mut phase);
...
fn test_datc_6_d_27() {
    ...
    phase.units.push(unit_e_swe);
    phase.units.push(unit_e_den);
    phase.units.push(unit_g_ber);
    phase.units.push(unit_r_bal);
    phase.units.push(unit_r_pru);
    phase.orders.push(unit_e_swe.move_to(p("bal")));
    phase.orders.push(unit_e_den.support_move(unit_e_swe, p("bal")));
    phase.orders.push(unit_g_ber.hold());
    phase.orders.push(unit_r_bal.convoy(unit_g_ber, p("lvn")));
    phase.orders.push(unit_r_pru.support_hold(unit_r_bal));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    ...
    assert_eq!(phase.orders[4].status, OrderStatus::Valid);
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&unit_e_swe));
...
fn test_datc_6_d_31() {
    ...
    phase.units.push(unit_a_rum);
    phase.units.push(unit_t_bla);
    phase.orders.push(unit_a_rum.move_to(p("arm")));
    phase.orders.push(unit_t_bla.support_move(unit_a_rum, p("arm")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Invalid);
    assert_eq!(phase.orders[1].status, OrderStatus::Invalid);
    assert_eq!(phase.units.len(), 2);
    assert!(phase.units.contains(&unit_a_rum));
...
fn test_datc_6_d_33() {
    ...
    phase.units.push(unit_a_ser);
    phase.units.push(unit_a_vie);
    phase.units.push(unit_r_gal);
    phase.units.push(unit_t_bul);
    phase.orders.push(unit_a_ser.move_to(p("bud")));
    phase.orders.push(unit_a_vie.move_to(p("bud")));
    phase.orders.push(unit_r_gal.support_move(unit_a_ser, p("bud")));
    phase.orders.push(unit_t_bul.move_to(p("ser")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Failure);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    assert_eq!(phase.orders[3].status, OrderStatus::Success);
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&a("a", "bud")));
    assert!(phase.units.contains(&unit_a_vie));
    assert!(phase.units.contains(&unit_r_gal));
    assert!(phase.units.contains(&a("t", "ser")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_d_34() {
    ...
    phase.units.push(unit_g_ber);
    phase.units.push(unit_g_sil);
    phase.units.push(unit_g_bal);
    phase.units.push(unit_i_pru);
    phase.units.push(unit_r_war);
    phase.units.push(unit_r_lvn);
    phase.orders.push(unit_g_ber.move_to(p("pru")));
    phase.orders.push(unit_g_sil.support_move(unit_g_ber, p("pru")));
    phase.orders.push(unit_g_bal.support_move(unit_g_ber, p("pru")));
    phase.orders.push(unit_i_pru.support_move(unit_i_pru, p("pru")));
    phase.orders.push(unit_r_war.support_move(unit_r_lvn, p("pru")));
    phase.orders.push(unit_r_lvn.move_to(p("pru")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    ...
    assert_eq!(phase.orders[5].status, OrderStatus::Failure);
    assert_eq!(phase.units.len(), 6);
    assert!(phase.units.contains(&a("g", "pru")));
    assert!(phase.units.contains(&unit_g_sil));
    assert!(phase.units.contains(&unit_g_bal));
    assert!(phase.units.contains(&unit_i_pru.set_dislodged_from(Some(p("ber")))));
    assert!(phase.units.contains(&unit_r_war));
    assert!(phase.units.contains(&unit_r_lvn));
    assert!(phase.standoff_codes.is_empty());
}
