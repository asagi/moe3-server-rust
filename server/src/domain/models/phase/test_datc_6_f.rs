...
fn test_datc_6_f_1() {
    ...
    let unit_t_gre = a("t", "gre");
    let unit_t_aeg = f("t", "aeg");
    let unit_t_con = f("t", "con");
    let unit_t_bla = f("t", "bla");
    phase.units.push(unit_t_gre);
    phase.units.push(unit_t_aeg);
    phase.units.push(unit_t_con);
    phase.units.push(unit_t_bla);
    phase.orders.push(unit_t_gre.move_to(p("sev")));
    phase.orders.push(unit_t_aeg.convoy(unit_t_gre, p("sev")));
    phase.orders.push(unit_t_con.convoy(unit_t_gre, p("sev")));
    phase.orders.push(unit_t_bla.convoy(unit_t_gre, p("sev")));
    ...
    assert_eq!(phase.orders[3].status, OrderStatus::Invalid);
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&unit_t_gre));
    assert!(phase.units.contains(&unit_t_aeg));
    assert!(phase.units.contains(&unit_t_con));
    assert!(phase.units.contains(&unit_t_bla));
...
fn test_datc_6_f_2() {
    ...
    let unit_f_par = a("f", "par");
    phase.units.push(unit_e_eng);
    phase.units.push(unit_e_lon);
    phase.units.push(unit_f_par);
    phase.orders.push(unit_e_eng.convoy(unit_e_lon, p("bre")));
    phase.orders.push(unit_e_lon.move_to(p("bre")));
    phase.orders.push(unit_f_par.move_to(p("bre")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert!(phase.units.contains(&unit_e_eng));
    assert!(phase.units.contains(&unit_e_lon));
    assert!(phase.units.contains(&unit_f_par));
    assert!(phase.standoff_codes.contains(&"bre".to_string()));
}
...
fn test_datc_6_f_3() {
    ...
    let unit_e_eng = f("e", "eng");
    let unit_e_lon = a("e", "lon");
    let unit_e_mid = f("e", "mid");
    let unit_f_par = a("f", "par");
    phase.units.push(unit_e_eng);
    phase.units.push(unit_e_lon);
    phase.units.push(unit_e_mid);
    phase.units.push(unit_f_par);
    phase.orders.push(unit_e_eng.convoy(unit_e_lon, p("bre")));
    phase.orders.push(unit_e_lon.move_to(p("bre")));
    phase.orders.push(unit_e_mid.support_move(unit_e_lon, p("bre")));
    phase.orders.push(unit_f_par.move_to(p("bre")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    assert_eq!(phase.orders[3].status, OrderStatus::Failure);
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&unit_e_eng));
    assert!(phase.units.contains(&a("e", "bre")));
    assert!(phase.units.contains(&unit_e_mid));
    assert!(phase.units.contains(&unit_f_par));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_4() {
    ...
    phase.units.push(unit_e_lon);
    phase.units.push(unit_g_ska);
    phase.orders.push(unit_e_nth.convoy(unit_e_lon, p("hol")));
    phase.orders.push(unit_e_lon.move_to(p("hol")));
    phase.orders.push(unit_g_ska.move_to(p("nth")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Failure);
    ...
    assert!(phase.units.contains(&a("e", "hol")));
    assert!(phase.units.contains(&unit_g_ska));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_5() {
    ...
    phase.units.push(unit_e_nth);
    phase.units.push(unit_e_lon);
    phase.units.push(unit_f_eng);
    phase.units.push(unit_f_bel);
    phase.units.push(unit_g_ska);
    phase.units.push(unit_g_den);
    phase.orders.push(unit_e_nth.convoy(unit_e_lon, p("hol")));
    phase.orders.push(unit_e_lon.move_to(p("hol")));
    phase.orders.push(unit_f_eng.move_to(p("nth")));
    phase.orders.push(unit_f_bel.support_move(unit_f_eng, p("nth")));
    phase.orders.push(unit_g_ska.move_to(p("nth")));
    phase.orders.push(unit_g_den.support_move(unit_g_ska, p("nth")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Failure);
    assert_eq!(phase.orders[3].status, OrderStatus::Valid);
    assert_eq!(phase.orders[4].status, OrderStatus::Failure);
    assert_eq!(phase.orders[5].status, OrderStatus::Valid);
    assert_eq!(phase.units.len(), 6);
    assert!(phase.units.contains(&unit_e_nth));
    assert!(phase.units.contains(&a("e", "hol")));
    assert!(phase.units.contains(&unit_f_eng));
    assert!(phase.units.contains(&unit_f_bel));
    assert!(phase.units.contains(&unit_g_ska));
    assert!(phase.units.contains(&unit_g_den));
    assert!(phase.standoff_codes.contains(&"nth".to_string()));
}
...
fn test_datc_6_f_6() {
    let mut phase = Phase::new_spring_main(1900, 1);
    let mut unit_e_nth = f("e", "nth");
    let unit_e_lon = a("e", "lon");
    let unit_g_hol = a("g", "hol");
    let unit_g_bel = a("g", "bel");
    let unit_g_hel = f("g", "hel");
    let unit_g_ska = f("g", "ska");
    let unit_f_pic = a("f", "pic");
    let unit_f_bur = a("f", "bur");
    phase.units.push(unit_e_nth);
    phase.units.push(unit_e_lon);
    phase.units.push(unit_g_hol);
    phase.units.push(unit_g_bel);
    phase.units.push(unit_g_hel);
    phase.units.push(unit_g_ska);
    phase.units.push(unit_f_pic);
    phase.units.push(unit_f_bur);
    phase.orders.push(unit_e_nth.convoy(unit_e_lon, p("hol")));
    phase.orders.push(unit_e_lon.move_to(p("hol")));
    phase.orders.push(unit_g_hol.support_hold(unit_g_bel));
    phase.orders.push(unit_g_bel.support_hold(unit_g_hol));
    phase.orders.push(unit_g_hel.support_move(unit_g_ska, p("nth")));
    phase.orders.push(unit_g_ska.move_to(p("nth")));
    phase.orders.push(unit_f_pic.move_to(p("bel")));
    phase.orders.push(unit_f_bur.support_move(unit_f_pic, p("bel")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[1].status, OrderStatus::Unreachable);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    assert_eq!(phase.orders[3].status, OrderStatus::Cut);
    assert_eq!(phase.orders[4].status, OrderStatus::Valid);
    assert_eq!(phase.orders[5].status, OrderStatus::Success);
    assert_eq!(phase.orders[6].status, OrderStatus::Failure);
    assert_eq!(phase.orders[7].status, OrderStatus::Valid);
    assert_eq!(phase.units.len(), 8);
    assert!(phase.units.contains(&unit_e_nth.set_dislodged_from(Some(p("ska")))));
    assert!(phase.units.contains(&unit_e_lon));
    assert!(phase.units.contains(&unit_g_hol));
    assert!(phase.units.contains(&unit_g_bel));
    assert!(phase.units.contains(&unit_g_hel));
    assert!(phase.units.contains(&f("g", "nth")));
    assert!(phase.units.contains(&unit_f_pic));
    assert!(phase.units.contains(&unit_f_bur));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_7() {
    ...
    assert_ne!(phase.orders[0].dislodged_from.map(|p| p.code_with_coast()), Some("hol"));
    assert!(!phase.standoff_codes.contains(&"hol".to_string()));
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&unit_e_nth.set_dislodged_from(Some(p("ska")))));
    assert!(phase.units.contains(&unit_e_lon));
    assert!(phase.units.contains(&unit_g_hel));
...
fn test_datc_6_f_8() {
    ...
    phase.orders.push(unit_g_hel.support_move(unit_g_ska, p("nth")));
    phase.orders.push(unit_g_ska.move_to(p("nth")));
    phase.orders.push(unit_g_bel.move_to(p("hol")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[1].status, OrderStatus::Unreachable);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    assert_eq!(phase.orders[3].status, OrderStatus::Success);
    assert_eq!(phase.orders[4].status, OrderStatus::Success);
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&unit_e_nth.set_dislodged_from(Some(p("ska")))));
    assert!(phase.units.contains(&unit_e_lon));
    assert!(phase.units.contains(&unit_g_hel));
    assert!(phase.units.contains(&f("g", "nth")));
    assert!(phase.units.contains(&a("g", "hol")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_9() {
    ...
    phase.units.push(unit_e_nth);
    phase.units.push(unit_e_lon);
    phase.units.push(unit_f_bre);
    phase.units.push(unit_f_mid);
    phase.orders.push(unit_e_eng.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_e_nth.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_e_lon.move_to(p("bel")));
    phase.orders.push(unit_f_bre.support_move(unit_f_mid, p("eng")));
    phase.orders.push(unit_f_mid.move_to(p("eng")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Dislodged);
    ...
    assert_eq!(phase.orders[4].status, OrderStatus::Success);
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&unit_e_eng.set_dislodged_from(Some(p("mid")))));
    assert!(phase.units.contains(&unit_e_nth));
    assert!(phase.units.contains(&a("e", "bel")));
    assert!(phase.units.contains(&unit_f_bre));
    assert!(phase.units.contains(&f("f", "eng")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_10() {
    ...
    let unit_e_nth = f("e", "nth");
    let unit_e_lon = a("e", "lon");
    let mut unit_g_eng = f("g", "eng");
    let unit_f_bre = f("f", "bre");
    let unit_f_mid = f("f", "mid");
    phase.units.push(unit_e_nth);
    phase.units.push(unit_e_lon);
    phase.units.push(unit_g_eng);
    phase.units.push(unit_f_bre);
    phase.units.push(unit_f_mid);
    phase.orders.push(unit_e_nth.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_e_lon.move_to(p("bel")));
    phase.orders.push(unit_g_eng.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_f_bre.support_move(unit_f_mid, p("eng")));
    phase.orders.push(unit_f_mid.move_to(p("eng")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    ...
    assert!(phase.units.contains(&unit_e_nth));
    assert!(phase.units.contains(&a("e", "bel")));
    assert!(phase.units.contains(&unit_g_eng.set_dislodged_from(Some(p("mid")))));
    assert!(phase.units.contains(&unit_f_bre));
    assert!(phase.units.contains(&f("f", "eng")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_11() {
    ...
    let unit_e_lon = a("e", "lon");
    let mut unit_g_eng = f("g", "eng");
    let unit_r_nth = f("r", "nth");
    let unit_f_bre = f("f", "bre");
    let unit_f_mid = f("f", "mid");
    phase.units.push(unit_e_lon);
    phase.units.push(unit_g_eng);
    phase.units.push(unit_r_nth);
    phase.units.push(unit_f_bre);
    phase.units.push(unit_f_mid);
    phase.orders.push(unit_e_lon.move_to(p("bel")));
    phase.orders.push(unit_g_eng.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_r_nth.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_f_bre.support_move(unit_f_mid, p("eng")));
    phase.orders.push(unit_f_mid.move_to(p("eng")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert!(phase.units.contains(&a("e", "bel")));
    assert!(phase.units.contains(&unit_g_eng.set_dislodged_from(Some(p("mid")))));
    assert!(phase.units.contains(&unit_r_nth));
    assert!(phase.units.contains(&unit_f_bre));
    assert!(phase.units.contains(&f("f", "eng")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_12() {
    ...
    let unit_e_eng = f("e", "eng");
    let unit_e_lon = a("e", "lon");
    let mut unit_e_iri = f("e", "iri");
    let unit_f_nat = f("f", "nat");
    let unit_f_mid = f("f", "mid");
    phase.units.push(unit_e_eng);
    phase.units.push(unit_e_lon);
    phase.units.push(unit_e_iri);
    phase.units.push(unit_f_nat);
    phase.units.push(unit_f_mid);
    phase.orders.push(unit_e_eng.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_e_lon.move_to(p("bel")));
    phase.orders.push(unit_e_iri.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_f_nat.support_move(unit_f_mid, p("iri")));
    phase.orders.push(unit_f_mid.move_to(p("iri")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Dislodged);
    ...
    assert!(phase.units.contains(&unit_e_eng));
    assert!(phase.units.contains(&a("e", "bel")));
    assert!(phase.units.contains(&unit_e_iri.set_dislodged_from(Some(p("mid")))));
    assert!(phase.units.contains(&unit_f_nat));
    assert!(phase.units.contains(&f("f", "iri")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_13() {
    ...
    phase.orders.push(unit_e_lon.move_to(p("bel")));
    phase.orders.push(unit_e_nth.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_f_eng.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_g_hol.support_move(unit_g_den, p("nth")));
    phase.orders.push(unit_g_den.move_to(p("nth")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    ...
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&a("e", "bel")));
    assert!(phase.units.contains(&unit_e_nth.set_dislodged_from(Some(p("den")))));
    assert!(phase.units.contains(&unit_f_eng));
    assert!(phase.units.contains(&unit_g_hol));
    assert!(phase.units.contains(&f("g", "nth")));
    assert!(phase.standoff_codes.is_empty());
...
fn test_datc_6_f_14() {
    ...
    let unit_e_wal = f("e", "wal");
    let unit_f_bre = a("f", "bre");
    let mut unit_f_eng = f("f", "eng");
    phase.units.push(unit_e_lon);
    phase.units.push(unit_e_wal);
    phase.units.push(unit_f_bre);
    phase.units.push(unit_f_eng);
    phase.orders.push(unit_e_lon.support_move(unit_e_wal, p("eng")));
    phase.orders.push(unit_e_wal.move_to(p("eng")));
    phase.orders.push(unit_f_bre.move_to(p("lon")));
    phase.orders.push(unit_f_eng.convoy(unit_f_bre, p("lon")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Unreachable);
    assert_eq!(phase.orders[3].status, OrderStatus::Dislodged);
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&unit_e_lon));
    assert!(phase.units.contains(&f("e", "eng")));
    assert!(phase.units.contains(&unit_f_bre));
    assert!(phase.units.contains(&unit_f_eng.set_dislodged_from(Some(p("wal")))));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_15() {
    ...
    let unit_f_bre = a("f", "bre");
    let mut unit_f_eng = f("f", "eng");
    let unit_i_iri = f("i", "iri");
    let unit_i_mid = f("i", "mid");
    let unit_i_naf = a("i", "naf");
    phase.units.push(unit_e_lon);
    phase.units.push(unit_e_wal);
    phase.units.push(unit_f_bre);
    phase.units.push(unit_f_eng);
    phase.units.push(unit_i_iri);
    phase.units.push(unit_i_mid);
    phase.units.push(unit_i_naf);
    phase.orders.push(unit_e_lon.support_move(unit_e_wal, p("eng")));
    phase.orders.push(unit_e_wal.move_to(p("eng")));
    phase.orders.push(unit_f_bre.move_to(p("lon")));
    phase.orders.push(unit_f_eng.convoy(unit_f_bre, p("lon")));
    phase.orders.push(unit_i_iri.convoy(unit_i_naf, p("wal")));
    phase.orders.push(unit_i_mid.convoy(unit_i_naf, p("wal")));
    phase.orders.push(unit_i_naf.move_to(p("wal")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Unreachable);
    ...
    assert_eq!(phase.orders[4].status, OrderStatus::Valid);
    assert_eq!(phase.orders[5].status, OrderStatus::Valid);
    assert_eq!(phase.orders[6].status, OrderStatus::Success);
    assert_eq!(phase.units.len(), 7);
    assert!(phase.units.contains(&unit_e_lon));
    assert!(phase.units.contains(&f("e", "eng")));
    assert!(phase.units.contains(&unit_f_bre));
    assert!(phase.units.contains(&unit_f_eng.set_dislodged_from(Some(p("wal")))));
    assert!(phase.units.contains(&unit_i_iri));
    assert!(phase.units.contains(&unit_i_mid));
    assert!(phase.units.contains(&a("i", "wal")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_16() {
    ...
    let unit_g_bel = f("g", "bel");
    phase.units.push(unit_e_lon);
    phase.units.push(unit_e_wal);
    phase.units.push(unit_f_bre);
    phase.units.push(unit_f_eng);
    phase.units.push(unit_g_nth);
    phase.units.push(unit_g_bel);
    phase.orders.push(unit_e_lon.support_move(unit_e_wal, p("eng")));
    phase.orders.push(unit_e_wal.move_to(p("eng")));
    phase.orders.push(unit_f_bre.move_to(p("lon")));
    phase.orders.push(unit_f_eng.convoy(unit_f_bre, p("lon")));
    phase.orders.push(unit_g_nth.support_move(unit_g_bel, p("eng")));
    phase.orders.push(unit_g_bel.move_to(p("eng")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Failure);
    assert_eq!(phase.orders[2].status, OrderStatus::Failure);
    assert_eq!(phase.orders[3].status, OrderStatus::Valid);
    assert_eq!(phase.orders[4].status, OrderStatus::Valid);
    assert_eq!(phase.orders[5].status, OrderStatus::Failure);
    assert_eq!(phase.units.len(), 6);
    assert!(phase.units.contains(&unit_e_lon));
    assert!(phase.units.contains(&unit_e_wal));
    assert!(phase.units.contains(&unit_f_bre));
    assert!(phase.units.contains(&unit_f_eng));
    assert!(phase.units.contains(&unit_g_nth));
    assert!(phase.units.contains(&unit_g_bel));
    assert!(phase.standoff_codes.contains(&"eng".to_string()));
}
...
fn test_datc_6_f_17() {
    let mut phase = Phase::new_spring_main(1900, 1);
    let mut unit_e_lon = f("e", "lon");
    let unit_e_wal = f("e", "wal");
    let unit_f_bre = a("f", "bre");
    let unit_f_eng = f("f", "eng");
    let unit_f_yor = f("f", "yor");
    let unit_g_nth = f("g", "nth");
    let unit_g_bel = f("g", "bel");
    phase.units.push(unit_e_lon);
    phase.units.push(unit_e_wal);
    ...
    phase.orders.push(unit_f_bre.move_to(p("lon")));
    phase.orders.push(unit_f_eng.convoy(unit_f_bre, p("lon")));
    phase.orders.push(unit_f_yor.support_move(unit_f_bre, p("lon")));
    phase.orders.push(unit_g_nth.support_move(unit_g_bel, p("eng")));
    phase.orders.push(unit_g_bel.move_to(p("eng")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Dislodged);
    ...
    assert_eq!(phase.orders[6].status, OrderStatus::Failure);
    assert_eq!(phase.units.len(), 7);
    assert!(phase.units.contains(&unit_e_lon.set_dislodged_via_convoy()));
    assert!(phase.units.contains(&unit_e_wal));
    assert!(phase.units.contains(&a("f", "lon")));
    assert!(phase.units.contains(&unit_f_eng));
    assert!(phase.units.contains(&unit_f_yor));
    assert!(phase.units.contains(&unit_g_nth));
    assert!(phase.units.contains(&unit_g_bel));
    assert!(phase.standoff_codes.contains(&"eng".to_string()));
}
...
fn test_datc_6_f_18() {
    ...
    phase.orders.push(unit_e_nth.convoy(unit_e_lon, p("bel")));
    phase.orders.push(unit_e_lon.move_to(p("bel")));
    phase.orders.push(unit_e_eng.support_move(unit_e_lon, p("bel")));
    phase.orders.push(unit_f_bel.support_hold(unit_e_nth));
    phase.orders.push(unit_g_hel.support_move(unit_g_ska, p("nth")));
    phase.orders.push(unit_g_ska.move_to(p("nth")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
...
fn test_datc_6_f_19() {
    let mut phase = Phase::new_spring_main(1900, 1);
    let unit_f_tun = a("f", "tun");
    let unit_f_tyn = f("f", "tyn");
    let unit_f_ion = f("f", "ion");
    let unit_i_nap = f("i", "nap");
    let unit_i_rom = f("i", "rom");
    phase.units.push(unit_f_tun);
    phase.units.push(unit_f_tyn);
    phase.units.push(unit_f_ion);
    phase.units.push(unit_i_nap);
    phase.units.push(unit_i_rom);
    phase.orders.push(unit_f_tun.move_to(p("nap")));
    phase.orders.push(unit_f_tyn.convoy(unit_f_tun, p("nap")));
    phase.orders.push(unit_f_ion.convoy(unit_f_tun, p("nap")));
    phase.orders.push(unit_i_nap.support_move(unit_i_rom, p("tyn")));
    phase.orders.push(unit_i_rom.move_to(p("tyn")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert_eq!(phase.orders[4].status, OrderStatus::Failure);
    assert_eq!(phase.units.len(), 5);
    assert!(phase.units.contains(&unit_f_tun));
    assert!(phase.units.contains(&unit_f_tyn));
    assert!(phase.units.contains(&unit_f_ion));
    assert!(phase.units.contains(&unit_i_nap));
...
fn test_datc_6_f_20() {
    ...
    let unit_f_tyn = f("f", "tyn");
    let unit_i_nap = f("i", "nap");
    let mut unit_i_ion = f("i", "ion");
    let unit_t_aeg = f("t", "aeg");
    let unit_t_eas = f("t", "eas");
    phase.units.push(unit_f_tun);
    phase.units.push(unit_f_tyn);
    phase.units.push(unit_i_nap);
    phase.units.push(unit_i_ion);
    phase.units.push(unit_t_aeg);
    phase.units.push(unit_t_eas);
    phase.orders.push(unit_f_tun.move_to(p("nap")));
    phase.orders.push(unit_f_tyn.convoy(unit_f_tun, p("nap")));
    phase.orders.push(unit_i_nap.support_hold(unit_i_ion));
    phase.orders.push(unit_i_ion.convoy(unit_f_tun, p("nap")));
    phase.orders.push(unit_t_aeg.support_move(unit_t_eas, p("ion")));
    phase.orders.push(unit_t_eas.move_to(p("ion")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    ...
    assert!(phase.units.contains(&unit_f_tyn));
    assert!(phase.units.contains(&unit_i_nap));
    assert!(phase.units.contains(&unit_i_ion.set_dislodged_from(Some(p("eas")))));
    assert!(phase.units.contains(&unit_t_aeg));
    assert!(phase.units.contains(&f("t", "ion")));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_21() {
    ...
    let unit_r_nwy = a("r", "nwy");
    let unit_f_iri = f("f", "iri");
    let unit_f_mid = f("f", "mid");
    let unit_e_lvp = a("e", "lvp");
    let mut unit_e_nat = f("e", "nat");
    let mut unit_e_cly = f("e", "cly");
    phase.units.push(unit_r_edi);
    phase.units.push(unit_r_nrg);
    phase.units.push(unit_r_nwy);
    phase.units.push(unit_f_iri);
    phase.units.push(unit_f_mid);
    phase.units.push(unit_e_lvp);
    phase.units.push(unit_e_nat);
    phase.units.push(unit_e_cly);
    phase.orders.push(unit_r_edi.support_move(unit_r_nwy, p("cly")));
    phase.orders.push(unit_r_nrg.convoy(unit_r_nwy, p("cly")));
    phase.orders.push(unit_r_nwy.move_to(p("cly")));
    phase.orders.push(unit_f_iri.support_move(unit_f_mid, p("nat")));
    phase.orders.push(unit_f_mid.move_to(p("nat")));
    phase.orders.push(unit_e_lvp.move_to(p("cly")));
    phase.orders.push(unit_e_nat.convoy(unit_e_lvp, p("cly")));
    phase.orders.push(unit_e_cly.support_hold(unit_e_nat));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    assert_eq!(phase.orders[3].status, OrderStatus::Valid);
    assert_eq!(phase.orders[4].status, OrderStatus::Success);
    assert_eq!(phase.orders[5].status, OrderStatus::Unreachable);
    assert_eq!(phase.orders[6].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[7].status, OrderStatus::Dislodged);
    assert_eq!(phase.units.len(), 8);
    assert!(phase.units.contains(&unit_r_edi));
    assert!(phase.units.contains(&unit_r_nrg));
    assert!(phase.units.contains(&a("r", "cly")));
    assert!(phase.units.contains(&unit_f_iri));
    assert!(phase.units.contains(&f("f", "nat")));
    assert!(phase.units.contains(&unit_e_lvp));
    assert!(phase.units.contains(&unit_e_nat.set_dislodged_from(Some(p("mid")))));
    assert!(phase.units.contains(&unit_e_cly.set_dislodged_via_convoy()));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_22() {
    ...
    let mut unit_f_eng = f("f", "eng");
    let unit_g_bel = f("g", "bel");
    let unit_g_pic = f("g", "pic");
    let unit_r_nwy = a("r", "nwy");
    let mut unit_r_nth = f("r", "nth");
    phase.units.push(unit_e_edi);
    phase.units.push(unit_e_lon);
    phase.units.push(unit_f_bre);
    phase.units.push(unit_f_eng);
    phase.units.push(unit_g_bel);
    phase.units.push(unit_g_pic);
    phase.units.push(unit_r_nwy);
    phase.units.push(unit_r_nth);
    phase.orders.push(unit_e_edi.move_to(p("nth")));
    phase.orders.push(unit_e_lon.support_move(unit_e_edi, p("nth")));
    phase.orders.push(unit_f_bre.move_to(p("lon")));
    phase.orders.push(unit_f_eng.convoy(unit_f_bre, p("lon")));
    phase.orders.push(unit_g_bel.support_move(unit_g_pic, p("eng")));
    phase.orders.push(unit_g_pic.move_to(p("eng")));
    phase.orders.push(unit_r_nwy.move_to(p("bel")));
    phase.orders.push(unit_r_nth.convoy(unit_r_nwy, p("bel")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Unreachable);
    ...
    assert_eq!(phase.orders[4].status, OrderStatus::Valid);
    assert_eq!(phase.orders[5].status, OrderStatus::Success);
    assert_eq!(phase.orders[6].status, OrderStatus::Unreachable);
    assert_eq!(phase.orders[7].status, OrderStatus::Dislodged);
    assert_eq!(phase.units.len(), 8);
    assert!(phase.units.contains(&f("e", "nth")));
    assert!(phase.units.contains(&unit_e_lon));
    assert!(phase.units.contains(&unit_f_bre));
    assert!(phase.units.contains(&unit_f_eng.set_dislodged_from(Some(p("pic")))));
    assert!(phase.units.contains(&unit_g_bel));
    assert!(phase.units.contains(&f("g", "eng")));
    assert!(phase.units.contains(&unit_r_nwy));
    assert!(phase.units.contains(&unit_r_nth.set_dislodged_from(Some(p("edi")))));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_23() {
    ...
    phase.units.push(unit_i_mid);
    phase.units.push(unit_i_iri);
    phase.units.push(unit_r_mwy);
    phase.units.push(unit_r_nth);
    phase.orders.push(unit_e_edi.move_to(p("nth")));
    phase.orders.push(unit_e_yor.support_move(unit_e_edi, p("nth")));
    phase.orders.push(unit_f_bre.move_to(p("lon")));
    phase.orders.push(unit_f_eng.convoy(unit_f_bre, p("lon")));
    phase.orders.push(unit_g_bel.support_hold(unit_f_eng));
    phase.orders.push(unit_g_lon.support_hold(unit_r_nth));
    phase.orders.push(unit_i_mid.move_to(p("eng")));
    phase.orders.push(unit_i_iri.support_move(unit_i_mid, p("eng")));
    phase.orders.push(unit_r_mwy.move_to(p("bel")));
    phase.orders.push(unit_r_nth.convoy(unit_r_mwy, p("bel")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    assert_eq!(phase.orders[1].status, OrderStatus::Valid);
    assert_eq!(phase.orders[2].status, OrderStatus::Failure);
    ...
    assert_eq!(phase.orders[6].status, OrderStatus::Failure);
    assert_eq!(phase.orders[7].status, OrderStatus::Valid);
    assert_eq!(phase.orders[8].status, OrderStatus::Failure);
    assert_eq!(phase.orders[9].status, OrderStatus::Valid);
    assert_eq!(phase.units.len(), 10);
    assert!(phase.units.contains(&unit_e_edi));
    assert!(phase.units.contains(&unit_e_yor));
    assert!(phase.units.contains(&unit_f_bre));
    assert!(phase.units.contains(&unit_f_eng));
    assert!(phase.units.contains(&unit_g_bel));
    assert!(phase.units.contains(&unit_g_lon));
    assert!(phase.units.contains(&unit_i_mid));
    assert!(phase.units.contains(&unit_i_iri));
    assert!(phase.units.contains(&unit_r_mwy));
    assert!(phase.units.contains(&unit_r_nth));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_f_24() {
    ...
    phase.orders.push(unit_e_edi.move_to(p("nth")));
    phase.orders.push(unit_e_lon.support_move(unit_e_edi, p("nth")));
    phase.orders.push(unit_e_iri.move_to(p("eng")));
    phase.orders.push(unit_e_mid.support_move(unit_e_iri, p("eng")));
    phase.orders.push(unit_f_bre.move_to(p("lon")));
    phase.orders.push(unit_f_eng.convoy(unit_f_bre, p("lon")));
    phase.orders.push(unit_f_bel.support_hold(unit_f_eng));
    phase.orders.push(unit_r_nwy.move_to(p("bel")));
    phase.orders.push(unit_r_nth.convoy(unit_r_nwy, p("bel")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    ...
    assert_eq!(phase.orders[4].status, OrderStatus::Failure);
    assert_eq!(phase.orders[5].status, OrderStatus::Valid);
    assert_eq!(phase.orders[6].status, OrderStatus::Valid);
    assert_eq!(phase.orders[7].status, OrderStatus::Unreachable);
    assert_eq!(phase.orders[8].status, OrderStatus::Dislodged);
    assert_eq!(phase.units.len(), 9);
    assert!(phase.units.contains(&f("e", "nth")));
    assert!(phase.units.contains(&unit_e_lon));
...
fn test_datc_6_f_25() {
    let mut phase = Phase::new_spring_main(1900, 1);
    let unit_g_1_ruh = a("g", "ruh");
    let mut unit_g_2_hol = a("g", "hol");
    let unit_g_3_den = a("g", "den");
    let unit_g_4_ska = f("g", "ska");
    let unit_g_5_fin = a("g", "fin");
    let unit_e_1_yor = a("e", "yor");
    let unit_e_2_nth = f("e", "nth");
    let unit_e_3_hel = f("e", "hel");
    let unit_e_4_bel = a("e", "bel");
    let unit_r_1_nrg = f("r", "nrg");
    let mut unit_r_2_nwy = f("r", "nwy");
    let unit_r_3_swe = f("r", "swe");
    phase.units.push(unit_g_1_ruh);
    phase.units.push(unit_g_2_hol);
    phase.units.push(unit_g_3_den);
    phase.units.push(unit_g_4_ska);
    phase.units.push(unit_g_5_fin);
    phase.units.push(unit_e_1_yor);
    phase.units.push(unit_e_2_nth);
    phase.units.push(unit_e_3_hel);
    phase.units.push(unit_e_4_bel);
    phase.units.push(unit_r_1_nrg);
    phase.units.push(unit_r_2_nwy);
    phase.units.push(unit_r_3_swe);
    phase.orders.push(unit_g_1_ruh.move_to(p("bel")));
    phase.orders.push(unit_g_2_hol.support_move(unit_g_1_ruh, p("bel")));
    phase.orders.push(unit_g_3_den.move_to(p("nwy")));
    phase.orders.push(unit_g_4_ska.convoy(unit_g_3_den, p("nwy")));
    phase.orders.push(unit_g_5_fin.support_move(unit_g_3_den, p("nwy")));
    phase.orders.push(unit_e_1_yor.move_to(p("hol")));
    phase.orders.push(unit_e_2_nth.convoy(unit_e_1_yor, p("hol")));
    phase.orders.push(unit_e_3_hel.support_move(unit_e_1_yor, p("hol")));
    phase.orders.push(unit_e_4_bel.hold());
    phase.orders.push(unit_r_1_nrg.move_to(p("nth")));
    phase.orders.push(unit_r_2_nwy.support_move(unit_r_1_nrg, p("nth")));
    phase.orders.push(unit_r_3_swe.move_to(p("ska")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    assert_eq!(phase.orders[1].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    ...
    assert_eq!(phase.orders[6].status, OrderStatus::Valid);
    assert_eq!(phase.orders[7].status, OrderStatus::Valid);
    assert_eq!(phase.orders[8].status, OrderStatus::Success);
    assert_eq!(phase.orders[9].status, OrderStatus::Failure);
    assert_eq!(phase.orders[10].status, OrderStatus::Dislodged);
    assert_eq!(phase.orders[11].status, OrderStatus::Failure);
    assert_eq!(phase.units.len(), 12);
    assert!(phase.units.contains(&unit_g_1_ruh));
    assert!(phase.units.contains(&unit_g_2_hol.set_dislodged_via_convoy()));
    assert!(phase.units.contains(&a("g", "nwy")));
    assert!(phase.units.contains(&unit_g_4_ska));
    assert!(phase.units.contains(&unit_g_5_fin));
    assert!(phase.units.contains(&a("e", "hol")));
    assert!(phase.units.contains(&unit_e_2_nth));
    assert!(phase.units.contains(&unit_e_3_hel));
    assert!(phase.units.contains(&unit_e_4_bel));
    assert!(phase.units.contains(&unit_r_1_nrg));
    assert!(phase.units.contains(&unit_r_2_nwy.set_dislodged_via_convoy()));
    assert!(phase.units.contains(&unit_r_3_swe));
    assert!(phase.standoff_codes.is_empty());
}
