...
fn test_datc_6_c_1() {
    ...
    phase.orders.push(unit_t_con.move_to(p("smy")));
    phase.orders.push(unit_t_smy.move_to(p("ank")));
    ...
    assert!(phase.units.contains(&a("t", "smy")));
    assert!(phase.units.contains(&a("t", "ank")));
...
fn test_datc_6_c_2() {
    ...
    phase.orders.push(unit_t_con.move_to(p("smy")));
    phase.orders.push(unit_t_smy.move_to(p("ank")));
    phase.orders.push(unit_t_bul.support_move(unit_t_ank, p("con")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
...
fn test_datc_6_c_3() {
    ...
    phase.orders.push(unit_t_con.move_to(p("smy")));
    phase.orders.push(unit_t_smy.move_to(p("ank")));
    phase.orders.push(unit_t_bul.move_to(p("con")));
    resolve_orders_for_main_phase(&mut phase);
    ...
    assert_eq!(phase.units.len(), 4);
    assert!(phase.units.contains(&unit_t_ank));
    assert!(phase.units.contains(&unit_t_con));
    assert!(phase.units.contains(&unit_t_smy));
    assert!(phase.units.contains(&unit_t_bul));
    assert!(phase.standoff_codes.contains(&"con".to_string()));
}
...
fn test_datc_6_c_4() {
    ...
    phase.units.push(unit_t_adr);
    phase.units.push(unit_i_nap);
    phase.orders.push(unit_a_tri.move_to(p("ser")));
    phase.orders.push(unit_a_ser.move_to(p("bul")));
    phase.orders.push(unit_t_bul.move_to(p("tri")));
    phase.orders.push(unit_t_aeg.convoy(unit_t_bul, p("tri")));
    phase.orders.push(unit_t_ion.convoy(unit_t_bul, p("tri")));
    phase.orders.push(unit_t_adr.convoy(unit_t_bul, p("tri")));
    phase.orders.push(unit_i_nap.move_to(p("ion")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Success);
    assert_eq!(phase.orders[1].status, OrderStatus::Success);
    assert_eq!(phase.orders[2].status, OrderStatus::Success);
    ...
    assert!(phase.units.contains(&a("a", "ser")));
    assert!(phase.units.contains(&a("t", "tri")));
    assert!(phase.units.contains(&unit_t_aeg));
    assert!(phase.units.contains(&unit_t_ion));
    assert!(phase.units.contains(&unit_t_adr));
...
fn test_datc_6_c_5() {
    ...
    phase.orders.push(unit_t_adr.convoy(unit_t_bul, p("tri")));
    phase.orders.push(unit_i_nap.move_to(p("ion")));
    phase.orders.push(unit_i_tun.support_move(unit_i_nap, p("ion")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    assert_eq!(phase.orders[1].status, OrderStatus::Failure);
    ...
    assert!(phase.units.contains(&unit_t_bul));
    assert!(phase.units.contains(&unit_t_aeg));
    assert!(phase.units.contains(&unit_t_ion.set_dislodged_from(Some(p("nap")))));
    assert!(phase.units.contains(&unit_t_adr));
    assert!(phase.units.contains(&f("i", "ion")));
    assert!(phase.units.contains(&unit_i_tun));
    assert!(phase.standoff_codes.is_empty());
}
...
fn test_datc_6_c_7() {
    ...
    phase.orders.push(unit_f_eng.convoy(unit_f_bel, p("lon")));
    phase.orders.push(unit_f_bel.move_to(p("lon")));
    phase.orders.push(unit_f_bur.move_to(p("bel")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Valid);
    assert_eq!(phase.orders[1].status, OrderStatus::Failure);
    assert_eq!(phase.orders[2].status, OrderStatus::Valid);
    ...
    assert!(phase.standoff_codes.contains(&"bel".to_string()));
}
...
fn test_datc_6_c_8() {
    ...
    phase.units.push(unit_a_ser);
    phase.orders.push(unit_t_con.move_to(p("bla")));
    phase.orders.push(unit_t_bul.move_to(p("con")));
    phase.orders.push(unit_t_smy.support_move(unit_t_bul, p("con")));
    phase.orders.push(unit_r_bla.move_to(p("bul_ec")));
    phase.orders.push(unit_a_ser.move_to(p("bul")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    assert_eq!(phase.orders[1].status, OrderStatus::Failure);
    ...
    assert!(phase.units.contains(&unit_a_ser));
    assert!(phase.standoff_codes.contains(&"bul".to_string()));
}
...
fn test_datc_6_c_9() {
    ...
    phase.units.push(unit_r_bla);
    phase.units.push(unit_a_ser);
    phase.units.push(unit_a_bul);
    phase.orders.push(unit_t_con.move_to(p("bla")));
    phase.orders.push(unit_t_smy.support_move(unit_a_bul, p("con")));
    phase.orders.push(unit_r_bla.move_to(p("bul_ec")));
    phase.orders.push(unit_a_ser.move_to(p("bul")));
    phase.orders.push(unit_a_bul.move_to(p("con")));
    resolve_orders_for_main_phase(&mut phase);
    assert_eq!(phase.orders[0].status, OrderStatus::Failure);
    ...
    assert!(phase.units.contains(&unit_r_bla));
    assert!(phase.units.contains(&unit_a_ser));
    assert!(phase.units.contains(&unit_a_bul));
    assert!(phase.standoff_codes.contains(&"bul".to_string()));
}
